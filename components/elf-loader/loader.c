...
//            MSG("  %08X %04X %04X %-20s %08X          %08X                    %s + %X", rel.r_offset, symEntry, relType, type2String(relType), relAddr, sym.st_value, name, rel.r_addend);
            ...
            MSG("  %08lX %04X %04X %-20s %08lX %08lX %08lX                    %s + %lX", rel.r_offset, symEntry, relType, type2String(relType), relAddr, symAddr, sym.st_value, name, rel.r_addend);
            r = -1;
        } else if(relocateSymbol(relAddr, relType, symAddr, sym.st_value, &from, &to) != 0) {
            ERR("  %08lX %04X %04X %-20s %08lX %08lX %08lX %08lX->%08lX %s + %lX", rel.r_offset, symEntry, relType, type2String(relType), relAddr, symAddr, sym.st_value, from, to, name, rel.r_addend);
            r = -1;
        } else {
            MSG("  %08lX %04X %04X %-20s %08lX %08lX %08lX %08lX->%08lX %s + %lX", rel.r_offset, symEntry, relType, type2String(relType), relAddr, symAddr, sym.st_value, from, to, name, rel.r_addend);
        }
    }
    return r;
#ifdef __linux
